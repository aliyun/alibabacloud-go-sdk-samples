...
func Cert (siteResponseBody *esa20240910.CreateSiteResponseBody, client *esa20240910.Client) (_result *esa20240910.ApplyCertificateResponseBody, _err error) {
  fmt.Printf("[INFO] %s\n", "Begin Call ApplyCertificate to create resource")
  applyCertificateRequest := &esa20240910.ApplyCertificateRequest{
    Domains: dara.String("101.gositecdn.cn"),
    SiteId: siteResponseBody.SiteId,
    Type: dara.String("lets_encrypt"),
  }
  applyCertificateResponse, _err := client.ApplyCertificate(applyCertificateRequest)
  if _err != nil {
    return _result, _err
  }

  fmt.Printf("[INFO] %s\n", "Call ApplyCertificate success, response: ")
  fmt.Printf("[INFO] %s\n", util.ToJSONString(applyCertificateResponse))
  _result = applyCertificateResponse.Body
  return _result , _err
}

func DestroyCert (siteResponseBody *esa20240910.CreateSiteResponseBody, applyCertificateResponseBody *esa20240910.ApplyCertificateResponseBody, client *esa20240910.Client) (_err error) {
  fmt.Printf("[INFO] %s\n", "Begin Call DeleteCertificate to destroy resource")
  result := applyCertificateResponseBody.Result
  for _, item := range result {
    id := dara.StringValue(item.Id)
    deleteCertificateRequest := &esa20240910.DeleteCertificateRequest{
      SiteId: siteResponseBody.SiteId,
      Id: dara.String(id),
    }
    deleteCertificateResponse, _err := client.DeleteCertificate(deleteCertificateRequest)
    if _err != nil {
      return _err
    }

    fmt.Printf("[INFO] %s\n", "Call DeleteCertificate success, response: ")
    fmt.Printf("[INFO] %s\n", util.ToJSONString(deleteCertificateResponse))
  }
  return _err
}
...
func _main (args []*string) (_err error) {
  ...
  certRespBody, _err := Cert(siteRespBody, esa20240910Client)
  if _err != nil {
    return _err
  }

  // destroy resource
  _err = DestroyCert(siteRespBody, certRespBody, esa20240910Client)
  if _err != nil {
    return _err
  }
  return _err
